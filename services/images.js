...
const uploadMultiple = async (files, body, context) => {
    ...
    if (image) {
        for (file of files) {
            ...
            image.gallery.push({ image: file.filename.replace(/ /g, ''), type: fileType })
        }
        await image.save()
    } else {
        for (file of files) {
            let fileType = file.mimetype.split('/')[0]
            gallery.push({ image: file.filename.replace(/ /g, ''), type: fileType })
        }
        model = {
            image: "",
            id: body.id,
...
const getFile = async (id, model, context) => {
    const log = context.logger.start(`services:images:getFile`);
    if (!id) {
        throw new Error("user id is required");
    }
    let user = await db.image.findOne({ id: { $eq: id } });
    user.baseUrl = "http://localhost/Vezo360/assets/images/";
    if (!user) {
