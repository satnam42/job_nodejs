...
const uploadDocs = async (id, files, context) => {
  ...
  if (
    job.constructionDocumentation != "" &&
    job.constructionDocumentation !== undefined
  ) {
  ...
  if (files[0].mimetype == "application/pdf") {
    job.constructionDocumentation =
      pdfUrl + "/" + job.constructionDocumentation;
  } else {
    job.constructionDocumentation =
      imageUrl + "/" + job.constructionDocumentation;
  }
  return job;
};
...
const getPopularJobs = async (model, context) => {
  ...
  const popularJobs = await db.job
    .find({})
    .sort({ category: -1 })
    .populate("category");
  log.end();
...
const setJObs = (model, job, context) => {
  ...
  if (
    model.constructionDocumentation !== "string" &&
    model.constructionDocumentation !== undefined
  ) {
...
const getJobsByuserId = async (id, context) => {
  const log = context.logger.start(`services:jobs:getJobsByuserId`);
  let job = await db.job
    .find({ user: id })
